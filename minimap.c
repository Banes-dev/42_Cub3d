...
/*   Updated: 2024/06/02 17:34:23 by mminet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	refresh_minimap(t_game_instance *game)
{
	...
	ymap = game->player_y - 10.0;
	y = 0;
	while (y < MINIMAP_HEIGHT)
	{
		x = 0; 
		xmap = (game->player_x - 5.0);
		while (x < MINIMAP_WIDTH)
		{
			...
			x++;
			xmap = (game->player_x - 5.0) + ((10.0 / width) * x);
		}
		y++;
		ymap = (game->player_y - 5.0) + ((10.0 / height) * y);
	}
	put_player(game);
}
